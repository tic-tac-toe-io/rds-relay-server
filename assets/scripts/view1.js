...
(function () {
  ...
  TerminalPanel = (function () {
    ...
    function TerminalPanel(id, username, token, cols, rows) {
      this.id = id;
    ...
    TerminalPanel.prototype.perform = function () {
      var self, id, username, token, cols, rows, xs, REQ_TTY, term, ref$, pathname, host, protocol, url, s;
      if (this.performed) {
        return;
      }
      self = this;
      this.performed = true;
      id = this.id, username = this.username, token = this.token, cols = this.cols, rows = this.rows;
      xs = $('#term-container');
      xs[0].hidden = false;
      xs = $('#terminal-title');
      xs[0].innerHTML = id + "";
  ...
  xs[0].onclick = function () {
    var xs, agentId, username, token, cols, rows, lms, terminal;
    xs = $('#login-agent-id');
    agentId = xs[0].value;
    xs = $('#login-username');
    username = xs[0].value;
    ...
    console.log("login " + agentId + " with username `" + username + "` and token `" + token + "`");
    lms = $('#loginModal');
    lms.modal('hide');
    xs = $('#dashboard');
    xs[0].hidden = true;
    terminal = window.terminal = new TerminalPanel(agentId, username, token, cols, rows);
